...
WHERE D.[Id] = DS.[DoctorId] AND S.[Id] = DS.[SpecializationId];
	...
	SELECT D.[Name] + ' ' + D.[Surname] AS 'Full name', S.[Name] AS 'Specialization'
	FROM [Doctors] AS D
	JOIN [DoctorsSpecializations] AS DS ON DS.[DoctorId] = D.[Id]
	JOIN [Specializations] AS S ON DS.[SpecializationId] = S.[Id]
--2. Вивести прізвища та зарплати (сума ставки та надбавки)
--лікарів, які не перебувають у відпустці.
SELECT D.[Surname], D.Salary+D.Premium 
FROM [Doctors] AS D, [Vacations] AS V
WHERE D.[Id] = V.[DoctorId] AND GETDATE() NOT BETWEEN V.[StartDate] AND V.[EndDate]
	-- варіант 2 --
	SELECT D.[Surname], D.Salary+D.Premium 
	FROM [Doctors] AS D
	JOIN [Vacations] AS V ON V.[DoctorId] = D.[Id]
	WHERE GETDATE() NOT BETWEEN V.[StartDate] AND V.[EndDate]
--3. Вивести назви палат, що знаходяться у відділенні “Intensive
--Treatment”.
SELECT W.[Name]
FROM [Wards] AS W, [Departments] AS D
WHERE D.[Id] = W.[DepartmentId] AND D.[Id] IN (4)
	-- варіант 2 --
	SELECT W.[Name]
	FROM [Wards] AS W
...
FROM [Departments] AS DP, [Donations] AS DN, [Sponsors] AS S
WHERE DP.[Id] = DN.[DepartmentId] AND S.[Id] = DN.[SponsorId] AND S.[Name] = 'Umbrella Corporation'
	-- варіант 2 --
	SELECT DISTINCT DP.[Name]
	FROM [Departments] AS DP
	JOIN [Donations] AS DN ON DN.[DepartmentId] = DP.[Id]
	JOIN [Sponsors] AS S ON DN.[SponsorId] = S.[Id]
	WHERE S.[Name] = 'Umbrella Corporation'
--5. Вивести всі пожертвування за останній місяць у вигляді:
--відділення, спонсор, сума пожертвування, дата пожертву-
--вання.
SELECT DP.[Name], S.[Name], DN.[Amount], DN.[Date]
FROM [Departments] AS DP, [Donations] AS DN, [Sponsors] AS S
WHERE DP.[Id] = DN.[DepartmentId] AND S.[Id] = DN.[SponsorId] AND DN.[Date] BETWEEN '2023.10.16' AND '2023.11.16'
	-- варіант 2 --
	SELECT DP.[Name], S.[Name], DN.[Amount], DN.[Date]
	FROM [Departments] AS DP
	JOIN [Donations] AS DN ON DN.[DepartmentId] = DP.[Id]
	JOIN [Sponsors] AS S ON DN.[SponsorId] = S.[Id]
	WHERE DN.[Date] BETWEEN '2023.10.16' AND '2023.11.16'
--6. Вивести прізвища лікарів із зазначенням відділень, у яких
--вони проводять обстеження. Необхідно враховувати об-
--стеження, що проводяться лише у будні дні.
SELECT D.[Surname], DP.[Name]
FROM [Departments] AS DP, [Doctors] AS D, [DoctorsExaminations] AS DE, [Examinations] AS E, [Wards] AS W
WHERE D.[Id] = DE.[DoctorId] AND E.[Id] = DE.[ExaminationId] AND W.[Id] = DE.[WardId] AND DP.[Id] = W.[DepartmentId]
AND E.[DayOfWeek] < 6



--7. Вивести назви палат та корпуси відділень, у яких прово-
--дить обстеження лікар “Helen Williams”.
SELECT W.[Name], DP.[Building]
FROM [DoctorsExaminations] AS DE, [Wards] AS W, [Departments] AS DP, [Doctors] AS D
WHERE D.[Id] = DE.[DoctorId] AND W.[Id] = DE.[WardId] AND DP.[Id] = W.[DepartmentId] AND (D.[Name] + ' ' + D.[Surname]) = 'Helen Williams' 



--8. Вивести назви відділень, які отримували пожертвування
--у розмірі більше 100000, із зазначенням їхніх лікарів.
SELECT DP.[Name] AS 'Department', D.[Name] + ' ' + D.[Surname] AS 'Doctors'
FROM [Departments] AS DP, [Donations] AS DN, [Doctors] AS D, [DoctorsSpecializations] AS DS
WHERE DP.[Id] = DN.[DepartmentId] AND DN.[Amount] >= 100000 AND D.[Id] = DS.[DoctorId] AND DP.[Id] = DS.[SpecializationId]
	-- варіант 2 --
	SELECT DP.[Name] AS 'Department', D.[Name] + ' ' + D.[Surname] AS 'Doctors'
	FROM [Departments] AS DP
	JOIN [Donations] AS DN ON DN.[DepartmentId] = DP.[Id]
	JOIN [DoctorsSpecializations] AS DS ON DS.[SpecializationId] = DP.[Id]
	JOIN [Doctors] AS D ON DS.[DoctorId] = D.[Id]
	WHERE DN.[Amount] >= 100000
--9. Вивести назви відділень, у яких є лікарі, які не отримують
--надбавки.
SELECT DP.[Name]
FROM [Departments] AS DP, [Doctors] AS D, [DoctorsSpecializations] AS DS
WHERE DP.[Id] = DS.[SpecializationId] AND D.[Id] = DS.[DoctorId] AND D.[Premium] = 0
	-- варіант 2 --
	SELECT DP.[Name]
	FROM [Departments] AS DP
	JOIN [DoctorsSpecializations] AS DS ON DS.[SpecializationId] = DP.[Id]
	JOIN [Doctors] AS D ON DS.[DoctorId] = D.[Id]
	WHERE D.[Premium] = 0
--10. Вивести назви спеціалізацій, що використовуються для
--лікування захворювань із ступенем тяжкості вище 3.
------------немає даних-------------
--11. Вивести назви відділень та назви захворювань, обстеження
--яких проводили за останні півроку.
------------немає даних-------------
--12. Вивести назви відділень та назви палат, у яких проводи-
--лися обстеження із заразливих захворювань.
------------немає даних-------------
DROP TABLE [Wards]
